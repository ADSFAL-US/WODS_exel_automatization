...
class CreateDatabaseDialog(tk.Toplevel):
    ...
    def _setup_ui(self):
        tk.Label(self, text="""Название базы данных без ".db":""").pack(pady=5)
        entry = tk.Entry(self, textvariable=self.db_name)
        entry.pack(pady=5)
...
class StartWindow(tk.Tk):
    ...
    def _setup_ui(self):
        ...
        list_frame = tk.Frame(self, width=200)
        list_frame.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)
        
        self.table_list = ttk.Treeview(list_frame, show="tree", selectmode="browse")
